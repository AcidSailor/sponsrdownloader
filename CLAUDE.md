...
- **`pkg/sponsr/`** — HTTP client for the Sponsr JSON API (`api/v2`).
  ...
  - The JSON transport (request building, bearer auth, decoding, typed errors) is delegated to [`restkit`](https://github.com/acidsailor/restkit); `NewClient` wires it via `WithHTTPClient`/`WithHook`. A `stripBodyHook` drops the `null` body restkit would otherwise attach to every GET.
